...
var appRouter = router({
  ...
  storage: router({
    getUploadUrl: adminProcedure2.input(z3.object({
      ...
      const path3 = `client-${input.clientId}/${Date.now()}_${safeName}`;
      const { data, error } = await storage.from("panel-assets").createSignedUploadUrl(path3);
      if (error || !data) {
        throw new TRPCError3({ code: "INTERNAL_SERVER_ERROR", message: `Error al generar URL de subida: ${error?.message ?? "sin respuesta"}` });
      }
      const publicUrl = `${ENV.supabaseUrl}/storage/v1/object/public/panel-assets/${path3}`;
      return { signedUrl: data.signedUrl, path: path3, publicUrl, ext };
    })
  }),
