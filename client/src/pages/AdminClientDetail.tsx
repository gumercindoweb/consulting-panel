    ...
    <div className="space-y-6">
      ...
      {phases.length === 0 && (
        <div className="sdt-card p-5">
          <p className="font-label text-xs tracking-widest mb-3" style={{ color: "var(--ambar)", letterSpacing: "3px" }}>
          ...
          <div className="flex flex-wrap gap-2">
            {SDT_PHASES.map((name, i) => (
              <button
                ...
                className="font-label text-xs px-3 py-2 rounded"
                style={{
      ...
      <div className="space-y-2">
        {phases.map((phase) => (
          <div key={phase.id} className="sdt-card p-4">
            {editingId === phase.id ? (
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { updatePhase.mutate({ id: phase.id, ...editData }); setEditingId(null); }} className="flex items-center gap-1 font-label text-xs px-3 py-1.5 rounded" style={{ background: "#4eba8a", color: "#0B0807", border: "none", cursor: "pointer", letterSpacing: "2px" }}>
                    <Save size={12} /> GUARDAR
                  </button>
                  <button onClick={() => setEditingId(null)} className="flex items-center gap-1 font-label text-xs px-3 py-1.5 rounded" style={{ background: "rgba(255,255,255,0.06)", color: "var(--gris)", border: "none", cursor: "pointer", letterSpacing: "2px" }}>
                    <X size={12} /> CANCELAR
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="font-body text-sm font-medium" style={{ color: "var(--creme)" }}>{phase.name}</p>
                  {phase.description && <p className="text-xs mt-0.5" style={{ color: "var(--gris)" }}>{phase.description}</p>}
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-label text-xs px-2 py-1 rounded" style={{ color: phase.status === "completed" ? "#4eba8a" : phase.status === "in_progress" ? "var(--ambar)" : "var(--gris)", background: "rgba(255,255,255,0.05)", letterSpacing: "2px" }}>
      ...
      {/* Add new */}
      <div className="sdt-card p-5">
        <p className="font-label text-xs tracking-widest mb-3" style={{ color: "var(--gris)", letterSpacing: "3px" }}>AGREGAR ETAPA</p>
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1 font-label text-xs px-4 py-2 rounded"
...
function MilestonesTab({ clientId }: { clientId: number }) {
  ...
  return (
    <div className="space-y-4">
      {milestones.map((m) => (
        <div key={m.id} className="sdt-card p-4 flex items-start justify-between gap-4">
          <div>
            <p className="font-body text-sm font-medium" style={{ color: "var(--creme)" }}>{m.title}</p>
      ...
      <div className="sdt-card p-5">
        <p className="font-label text-xs tracking-widest mb-4" style={{ color: "var(--gris)", letterSpacing: "3px" }}>AGREGAR HITO</p>
...
function OKRsTab({ clientId }: { clientId: number }) {
  ...
  return (
    <div className="space-y-4">
      {okrs.map((okr) => (
        <div key={okr.id} className="sdt-card p-4">
          <div className="flex items-start justify-between gap-4 mb-2">
            <div>
              <p className="text-xs" style={{ color: "var(--gris)" }}>{okr.objective}</p>
              <p className="font-body text-sm font-medium" style={{ color: "var(--creme)" }}>{okr.keyResult}</p>
            </div>
      ...
      <div className="sdt-card p-5">
        <p className="font-label text-xs tracking-widest mb-4" style={{ color: "var(--gris)", letterSpacing: "3px" }}>AGREGAR OKR</p>
        <div className="grid grid-cols-2 gap-3 mb-3">
...
function LearningsTab({ clientId }: { clientId: number }) {
  ...
  return (
    <div className="space-y-4">
      {learnings.map((l) => (
        <div key={l.id} className="sdt-card p-4 flex items-start justify-between gap-4">
          <div>
            <span className="font-label text-xs" style={{ color: l.type === "win" ? "#4eba8a" : l.type === "obstacle" ? "var(--ambar)" : "#4db6e8", letterSpacing: "2px" }}>
              {l.type === "win" ? "LOGRO" : l.type === "obstacle" ? "OBSTÁCULO" : "APRENDIZAJE"}
            </span>
            <p className="font-body text-sm font-medium mt-1" style={{ color: "var(--creme)" }}>{l.title}</p>
      ...
      <div className="sdt-card p-5">
        <p className="font-label text-xs tracking-widest mb-4" style={{ color: "var(--gris)", letterSpacing: "3px" }}>AGREGAR REGISTRO</p>
        <div className="grid grid-cols-2 gap-3 mb-3">
...
function ScopeTab({ clientId }: { clientId: number }) {
  ...
  return (
    <div className="space-y-4">
      {items.map((item) => (
        <div key={item.id} className="sdt-card p-4 flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <span className="font-label text-xs px-2 py-1 rounded" style={{ color: item.inScope ? "#4eba8a" : "var(--rojo)", background: item.inScope ? "rgba(78,186,138,0.1)" : "rgba(179,40,37,0.1)", letterSpacing: "2px" }}>
              {item.inScope ? "INCLUIDO" : "EXCLUIDO"}
            </span>
            <div>
              <p className="font-body text-sm font-medium" style={{ color: "var(--creme)" }}>{item.title}</p>
      ...
      <div className="sdt-card p-5">
        <p className="font-label text-xs tracking-widest mb-4" style={{ color: "var(--gris)", letterSpacing: "3px" }}>AGREGAR ÍTEM DE ALCANCE</p>
        <div className="grid grid-cols-2 gap-3 mb-3">
...
function ResourcesTab({ clientId }: { clientId: number }) {
  ...
  return (
    <div className="space-y-4">
      {resources.map((r) => (
        <div key={r.id} className="sdt-card p-4 flex items-start justify-between gap-4">
          <div>
            <span className="font-label text-xs" style={{ color: "var(--ambar)", letterSpacing: "2px" }}>{r.category.toUpperCase()}</span>
            <p className="font-body text-sm font-medium mt-1" style={{ color: "var(--creme)" }}>{r.title}</p>
      ...
      <div className="sdt-card p-5">
        <p className="font-label text-xs tracking-widest mb-4" style={{ color: "var(--gris)", letterSpacing: "3px" }}>AGREGAR RECURSO</p>
        <div className="grid grid-cols-2 gap-3 mb-3">
...
function MetricsTab({ clientId }: { clientId: number }) {
  ...
  return (
    <div className="space-y-4">
      {metrics.map((m) => (
        <div key={m.id} className="sdt-card p-4 flex items-start justify-between gap-4">
          <div className="flex-1">
            <p className="font-label text-xs tracking-widest" style={{ color: "var(--gris)", letterSpacing: "3px" }}>{m.name.toUpperCase()}</p>
            <div className="flex items-baseline gap-2 mt-1">
              <input
                value={m.value}
                onChange={(e) => updateMetric.mutate({ id: m.id, clientId, value: e.target.value })}
                className="font-display text-2xl font-bold w-28 bg-transparent border-b"
                style={{ color: "var(--ambar)", borderColor: "rgba(255,255,255,0.1)", outline: "none", fontFamily: "var(--font-display)" }}
              />
      ...
      <div className="sdt-card p-5">
        <p className="font-label text-xs tracking-widest mb-4" style={{ color: "var(--gris)", letterSpacing: "3px" }}>AGREGAR MÉTRICA</p>
        <div className="grid grid-cols-2 gap-3 mb-3">
    ...
    <div className="min-h-screen" style={{ background: "var(--noir)" }}>
      ...
      >
        <button
          ...
          style={{ background: "none", border: "none", cursor: "pointer", color: "var(--gris)" }}
        >
          <ArrowLeft size={16} />
          <span className="font-label text-xs tracking-widest" style={{ letterSpacing: "2px" }}>VOLVER</span>
        </button>
        <div className="w-px h-6" style={{ background: "rgba(255,255,255,0.1)" }} />
        <div>
          <p className="font-label text-xs tracking-widest" style={{ color: "var(--ambar)", letterSpacing: "4px" }}>
            GESTIÓN DE CLIENTE
          </p>
          <h1 className="font-display text-xl font-bold" style={{ color: "var(--creme)" }}>
            {client.name}
          </h1>
        </div>
        <div className="ml-auto">
          <a
            ...
            className="flex items-center gap-2 font-label text-xs px-4 py-2 rounded"
            style={{ background: "rgba(255,255,255,0.05)", color: "var(--gris)", border: "1px solid rgba(255,255,255,0.1)", textDecoration: "none", letterSpacing: "2px" }}
          >
            VER COMO CLIENTE
          </a>
        </div>
        ...
        >
          {TABS.map((tab) => (
            <button
              ...
              className="font-label text-xs tracking-widest px-4 py-3 whitespace-nowrap"
              style={{
                background: "none",
                border: "none",
                cursor: "pointer",
                color: activeTab === tab.id ? "var(--creme)" : "var(--gris)",
                borderBottom: activeTab === tab.id ? "2px solid var(--rojo)" : "2px solid transparent",
                letterSpacing: "3px",
                marginBottom: "-1px",
              }}
            >
